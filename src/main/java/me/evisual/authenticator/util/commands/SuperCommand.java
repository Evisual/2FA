...
import me.evisual.authenticator.util.arrays.ArraysUtils;
...
{
    public SuperCommand(@NotNull String command, @Nullable String permission)
    {
            ...
            if (super.hasPermission(commandSender)) {
                ...
                for (Command subCmd : getSubCommands()) {
                    if (subCmd.getPermission() == null || commandSender.hasPermission(subCmd.getPermission())) {
                        if (subCmd.getCommand().toLowerCase().startsWith(args[0].toLowerCase()))
                            options.add(subCmd.getCommand());
